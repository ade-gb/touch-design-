...
const els = {
  stage: required("#gestureStage"),
  video: required("#cameraFeed"),
  canvas: required("#handCanvas"),
  cursor: required("#handCursor"),
  card: required("#productCard"),
  image: required("#productImage"),
  category: required("#productCategory"),
  index: required("#productIndex"),
  name: required("#productName"),
  description: required("#productDescription"),
  price: required("#productPrice"),
  signal: required("#productSignal"),
  variants: required("#variantRow"),
  status: required("#statusText"),
  cameraButton: required("#cameraButton"),
  fallbackButton: required("#fallbackButton"),
  zones: [...document.querySelectorAll(".gesture-zone")],
  brandRail: required("#brandRail"),
  cartItems: required("#cartItems"),
  cartTotal: required("#cartTotal"),
  checkoutButton: required("#checkoutButton"),
...
function renderProduct(direction) {
  ...
  els.variants.innerHTML = product.variants
    ...
    .join("");
  els.variants.querySelectorAll("button").forEach((button) => {
...
function renderBrands() {
  els.brandRail.innerHTML = brands
    ...
    .join("");
  els.brandRail.querySelectorAll("button").forEach((button) => {
    button.addEventListener("click", () => {
...
function renderCart() {
  ...
  const totals = state.cart.reduce((sum, item) => {
    const product = products.find((candidate) => candidate.id === item.productId);
...
function updateHand(x, y, visible = true) {
...
function updateHold(action, now) {
  if (!action) {
    state.holdStartedAt = 0;
    state.holdCompletedAction = null;
    setHoldProgress(0);
...
function trackSwipe(hand, action, now) {
